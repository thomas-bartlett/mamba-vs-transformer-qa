...
def preprocess_original(cfg, tokenizer):
    ...
    def preprocess_function(examples):
        ...
        inputs = [f"context: {c} question: {q} answer:" for q, c in zip(questions, contexts)]
        labels = [f"context: {c} question: {q} answer: {a}" for q, c, a in zip(questions, contexts, answers)]
    ...
    def preprocess_test_function(examples):
        questions = [q.strip() for q in examples["question"]]
        contexts = [c.strip() for c in examples["context"]]
        prompts = [f"context: {c} question: {q} answer:" for q, c in zip(questions, contexts)]

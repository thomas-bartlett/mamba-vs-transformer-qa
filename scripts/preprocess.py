...
def preprocess(cfg, tokenizer):
    ...
    train_test_split = dataset['train'].train_test_split(test_size=0.10, seed=42)
    ...
    def preprocess_test_function(examples):
        ...
        contexts = [c.strip() for c in examples["context"]]
        prompts = [f"context: {c} \n\nquestion: {q} \nanswer: " for q, c in zip(questions, contexts)]
        inputs = tokenizer(prompts, max_length=512, truncation=True, padding=False)
